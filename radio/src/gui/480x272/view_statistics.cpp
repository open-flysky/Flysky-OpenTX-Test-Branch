...
bool menuStatsAnalogs(event_t event)
{
  ...
  // RAS
  if ((isModuleXJT(INTERNAL_MODULE) && IS_INTERNAL_MODULE_ON())) {
  ...
  if (isModulePXX(EXTERNAL_MODULE)) {
    lcdDrawText(MENUS_MARGIN_LEFT, MENU_CONTENT_TOP+7*FH, "Ext. RAS");
    lcdDrawNumber(MENUS_MARGIN_LEFT+100, MENU_CONTENT_TOP+7*FH, telemetryData.swrExternal.value()));
  }
