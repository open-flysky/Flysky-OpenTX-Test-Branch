...
void RadioBluetoothPage::build(Window * window)
{
  ...
  new Choice(window, grid.getFieldSlot(), STR_BLUETOOTH_MODES, BLUETOOTH_FIRST, BLUETOOTH_LAST, GET_DEFAULT(g_eeGeneral.bluetoothMode),  [=](int32_t newValue) { 
  ...
  new CheckBox(window, slotCheck, isPasscodeEnabled, setBtPasscodeEnabled);
  sprintf(reusableBuffer.bt.pin, "%06lu", getBtPasscode());
  ...
  new Choice(window, grid.getFieldSlot(), STR_BLUETOOTH_PLATFORMS, BLUETOOTH_TARGET_PLATFORM_FIRST, BLUETOOTH_TARGET_PLATFORM_LAST, getBtPlatfrom, setBtPlatform);
  grid.nextLine();

  new StaticText(window, grid.getLabelSlot(), STR_BAUDRATE);
  new Choice(window, grid.getFieldSlot(), STR_BLUETOOTH_BAUDRATE, 0, 9, GET_DEFAULT(g_eeGeneral.bluetoothBaudrate), setBtBaudrate);

  grid.nextLine();

  new StaticText(window, grid.getLabelSlot(), STR_POWER);
  auto power = new Choice(window, grid.getFieldSlot(), "", btle::TxPower::power_first, btle::TxPower::power_last, GET_DEFAULT(bluetooth.config.tx_power), setBtTxPower);
