...
{
  TRACE("mixerSchedulerStart %d", getMixerSchedulerPeriod());
...
bool isMixerSchedulerStarted() {
  return (MIXER_SCHEDULER_TIMER->DIER & TIM_DIER_UIE) != 0;
}

void mixerSchedulerStop()
{
  TRACE("mixerSchedulerStop");
  MIXER_SCHEDULER_TIMER->CR1 &= ~TIM_CR1_CEN;
