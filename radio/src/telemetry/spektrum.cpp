 ...
 */
void processDSMBindPacket(const uint8_t *packet, uint8_t module)
{
  uint32_t debugval;
...
void processSpektrumTelemetryData(uint8_t data, uint8_t module, uint8_t* rxBuffer, uint8_t& rxBufferCount)
{
  ...
  if (rxBuffer[1] == 0x80 && rxBufferCount >= DSM_BIND_PACKET_LENGTH) {
    processDSMBindPacket(rxBuffer+2, module);
    rxBufferCount = 0;
    return;
  }
